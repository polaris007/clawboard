...
CREATE TABLE IF NOT EXISTS dashboard_scan_history (
    ...
    agg_hours_from_files INT       NOT NULL DEFAULT 0 COMMENT '从文件提取的小时数',
    agg_hours_from_window INT      NOT NULL DEFAULT 0 COMMENT '从时间窗口获取的小时数',
    agg_hours_total  INT           NOT NULL DEFAULT 0 COMMENT '聚合的总小时数（去重后）',
    agg_duration_ms  BIGINT        NOT NULL DEFAULT 0 COMMENT '聚合耗时(ms)',
...
CREATE TABLE IF NOT EXISTS dashboard_scan_progress (
    ...
    session_id      VARCHAR(100)  COMMENT '当前Session UUID',
...
CREATE TABLE IF NOT EXISTS dashboard_session_summary (
    session_id       VARCHAR(100)  NOT NULL COMMENT 'Session UUID/ID',
    employee_id      VARCHAR(50)   NOT NULL COMMENT '工号',
...
-- ============================================
-- 10. dashboard_execution_trace - Execution trace nodes
-- ============================================
CREATE TABLE IF NOT EXISTS dashboard_execution_trace (
    ...
    success         TINYINT(4)    COMMENT '是否成功（1=成功，0=失败）',
